...
# D: is the vacuity gap a resolution artefact? (CPU, loads checkpoints, ~30 min)
python run_depth.py

# S: does the framework transfer to a second domain? (numpy only, ~15 min)
python run_sorting.py
python run_sorting.py --smoke     # SMOKE seed; reports nothing
```
...
| `sorting.py` | The **second domain**. Six sort executors, `leftward_depth`, array targets, a ten-probe intervention class, `SORT_DOMAIN`. Nothing downstream of the intervention class is duplicated — see `fingerprint.Domain`. |
...
1. **Probe graphs must have weighted propagation depth ≥ 6, and ≥ 10 for neural work.** Use `fpid.graphs.weighted_depth`, not `hop_distances().max()` — relaxation follows minimum-*weight* paths, which use more edges than minimum-*hop* ones, so the two differ by 1–2 rounds and only coincide on a spanning path. Below weighted depth ≈ 5 no intervention set separates all pairs; dense shallow graphs return a null about nothing. Bucketing on the hop axis is what broke the original §5.2 result (`FINDINGS_P2_WITNESS.md`). `deep_probe_graphs`'s `--density 0.05 --min-depth 5` default gives weighted ~6, which is one step above the floor and measurably noisy; use `--density 0.02 --min-depth 8` (weighted ~10) for anything neural (`FINDINGS_DEPTH.md`).
...
- **Direction replicating while resolution does not is the characteristic signature of a dead lead** — this is what both `overshoot` and E3b's truncation statistic looked like. Leave no intermediate rung in the outcome ladder for that state to land on. Experiment S is the third instance: bubble/cocktail differ on 17 of 70 slots and all 17 are sub-threshold.
- **A probe distribution chosen by analogy to another domain is an untested assumption.** Experiment S's ladder failed because the sorting instance distribution was set deep, on the SSSP reasoning that depth buys resolving power. In sorting it *costs* resolving power. Sweep `D` in a new domain; never inherit it.
- **Targets must name what the algorithm cannot permute.** A node keeps its identity under relaxation, so a position is a stable handle in SSSP. A sort permutes positions, so a probe targeted at a *position* silently becomes a different probe by firing time — this broke Experiment S's null control (`FINDINGS_SORT.md` §11).

## Current project status (as of 2026-08-23)

Symbolic work: complete and positive. Neural work: finished and negative on all tested approaches, with the three remaining objections closed: E5 shows the vacuity gap is not a sensitivity-normalisation artefact; E6's attempt at a sharper Dijkstra/Prim probe failed its own mechanism check informatively (edge-weight raises are as invisible to relaxation as deletions, for the same monotonicity reason); **H repaired the instrument in both places it was known broken** — the `r = -0.233` orthogonality verdict is UNSUPPORTED (it is -0.004 under separating power, and the published value has p = 0.54), and the firing rule was poking networks at ~11% of their active life rather than 50%; fixing it leaves the vacuity gap UNCHANGED, which strengthens Proposition 5. And **G closed the reference-free question** — the fingerprints do not separate the networks from *each other* either (chance purity on 12 fresh seeds, 0/8 on gated-vs-plain, 11 cross-arm pairs bit-identical), which removes the last defence available to E3's null, that the label rather than the instrument was broken. And **D closed the resolution objection** — every neural experiment gated probe graphs on *hop* depth, which sits at weighted depth ~6 where `|S_min| = 3`, one step above the regime where nothing separates; re-measured at weighted depth ~10 where `|S_min| = 2` the vacuity gap does not close but **widens** (1.38 → 1.85 on the nine, 1.33 → 1.68 on the twelve; 0/9 and 2/12 against thresholds of 7/9 and 10/12), with the control arm reproducing H2's published 1.34. Nothing prior is invalidated (weighted depth is provably ≥ hop depth) but neural probing should use `--density 0.02 --min-depth 8`. And **S added the second domain** — comparison sorting, where four algorithms are output-identical on *every* input (400/400) and truncated bubble sort is correct iff `leftward_depth <= k` (139/139 vs 0/261), reproducing §5.2's exact step in a new domain. The machinery transferred mechanically (the `Domain` seam is proven inert: two SSSP runners are byte-identical across it) but the **pre-registered ladder failed**, 9 of 10 pairs separating, because the probe distribution was imported from SSSP and the depth heuristic *inverts* — resolving power falls with disorder here, and the [4,8] band separates all ten pairs. The recommended next action is writing up. See `HANDOFF.md` for the full status table and `ROUTES_TO_POSITIVE.md` for the ranked menu of what remains open — its §2.1 + §2.2 were both executed by H, so the standing top item is §5.1 (Family-2, state-dependent gain). The `FINDINGS_RESIDUAL.md` exploratory follow-up carries its own frozen confirmatory design and is explicitly not yet a result.
